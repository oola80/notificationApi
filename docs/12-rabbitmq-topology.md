...
| `xch.notifications.status` | topic | notification-engine-service, channel-router-service, template-service | Notification lifecycle status transitions, delivery status updates, and template audit events |
...
| `q.engine.status.inbound` | yes | `xch.notifications.dlq` | `x-dead-letter-exchange` | Inbound delivery status updates from Channel Router |
...
| `q.deliver.push.critical` | yes | `xch.notifications.dlq` | `x-dead-letter-exchange` | Critical push (FCM) delivery |
| `q.deliver.push.normal` | yes | `xch.notifications.dlq` | `x-dead-letter-exchange` | Normal push (FCM) delivery |
...
### 5.4 `xch.notifications.status` (topic) — 4 bindings
