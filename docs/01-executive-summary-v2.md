...
> **Note:** This is v2 of the Executive Summary, updated for the decoupled provider adapter architecture. The v1 reference is at `01-executive-summary.md`.
...
A unified channel routing layer abstracts away provider-specific integrations through independent provider adapter microservices. Adding a new channel or switching providers (e.g., from SendGrid to Amazon SES) requires no changes to the core routing service or any upstream service. Adding a new provider requires only deploying a small adapter service (~250 LOC) and registering it -- no changes to the core routing service or any upstream service.
...
| **Compute Layer** | Core business logic: event normalization, notification orchestration, template rendering, channel routing, email ingestion, administration | event-ingestion-service, notification-engine-service, template-service, channel-router-service, provider adapter services, admin-service, audit-service, email-ingest-service |
...
+------------------+      +-------------------+      +-------------+      +-------------------+      +-----------------+      +-------------------+      +-----------+
|  Source Systems   |      |  Event Ingestion  |      |  RabbitMQ   |      |   Notification    |      | Channel Router  |      | Provider Adapters |      | Customers |
|                   | ---> |                   | ---> |             | ---> |      Engine       | ---> |     (Core)      | ---> |                   | ---> |           |
|  - OMS            |      |  Normalize &      |      |  Message    |      |                   |      | Orchestration,  |      | SendGrid Adapter  |      |           |
|  - Magento 2      |      |  Validate         |      |  Broker     |      | Rules &           |      | Circuit Breaker,|      | Mailgun Adapter   |      |           |
|  - Mirakl         |      |  REST + RabbitMQ  |      |             |      | Orchestration     |      | Rate Limiting,  |      | Braze Adapter     |      |           |
|  - Chat Commerce  |      |                   |      |             |      | Template Rendering|      | Retry, Fallback |      | Twilio Adapter    |      |           |
|  - Manual         |      |                   |      |             |      | Recipient         |      |                 |      | FCM Adapter       |      |           |
...
> **Note:** Source systems are decoupled from the notification platform via RabbitMQ. They publish business events without knowledge of downstream notification logic. The Notification Engine applies configurable rules to determine what, when, and how to notify customers. The Channel Router Core delegates final delivery to independent provider adapter microservices, each responsible for a single provider's API integration.
...
5. **Channel Routing:** The Channel Router Core receives the rendered message and routes it to the appropriate provider adapter service via HTTP, which handles the final delivery to the external provider. The core service owns orchestration concerns (circuit breaker, rate limiting, retry, fallback), while each adapter handles only provider-specific API calls and webhook verification.
...
| **SendGrid** | Email Provider | Third-party email delivery service for transactional and marketing emails. Provides delivery tracking, bounce handling, and analytics. Integrated via a dedicated provider adapter microservice. |
| **Mailgun** | Email Provider | Third-party email delivery service offering an alternative to SendGrid for transactional emails. Integrated via a dedicated provider adapter microservice, enabling provider fallback and cost optimization. |
| **Braze** | Multi-Channel Provider | Unified multi-channel provider supporting email, SMS, WhatsApp, and push notifications. Used as a dumb pipe for pre-rendered content delivery via a dedicated provider adapter microservice. |
| **Twilio** | SMS / WhatsApp Provider | Third-party provider for SMS and WhatsApp message delivery. Supports message templating, delivery receipts, and two-way messaging via a dedicated provider adapter microservice. |
| **Firebase Cloud Messaging (FCM)** | Push Notification Provider | Google's push notification service for delivering notifications to mobile devices (iOS and Android) and web browsers. Integrated via a dedicated provider adapter microservice for real-time customer alerts. |
...
The platform is decomposed into a set of microservices, each with a single well-defined responsibility. Services communicate asynchronously via RabbitMQ for event-driven flows and synchronously via REST for query and configuration operations. The channel-router-service follows a decoupled provider adapter architecture: the core service handles orchestration while independent adapter microservices handle provider-specific delivery.
...
| **channel-router-service** | Delegates delivery to independent provider adapter microservices (SendGrid, Mailgun, Braze, Twilio, FCM). Core handles orchestration (circuit breaker, rate limiting, retry, fallback); adapters handle provider-specific API calls and webhook verification. | NestJS | RabbitMQ (inbound), REST (outbound to provider adapters) |
...
The microservices architecture allows individual services to scale independently based on load. During peak events (flash sales, order surges), the Channel Router, provider adapter services, and Notification Engine can scale out while other services remain at baseline capacity, optimizing infrastructure costs.
...
The decoupled provider adapter architecture allows switching or adding delivery providers without affecting the core routing service or any upstream service. Each provider is encapsulated in its own adapter microservice (~250 LOC), which can be deployed, updated, or replaced independently. Adding a new provider requires only deploying a small adapter service and registering it -- no changes to the core routing service or any upstream service. This eliminates vendor lock-in and enables cost optimization through competitive provider selection.
...
| Area | Details |
|---|---|
| **Notification Channels** | Email (via SendGrid, Mailgun, Braze), SMS (via Twilio, Braze), WhatsApp (via Twilio, Braze), Push Notifications (via Firebase Cloud Messaging, Braze) |
...
| **Platform Infrastructure** | All backend microservices, provider adapter microservices, RabbitMQ message broker setup, PostgreSQL databases (per-service), Docker containerization, API documentation |
...
| **Provider Rate Limits** | SendGrid, Mailgun, Braze, Twilio, and FCM impose rate limits on API calls. High-volume notification bursts may be throttled. | The Channel Router Core implements rate-limiting queues, exponential backoff retry, and circuit breakers to manage provider throughput constraints gracefully. Individual provider adapters respect provider-specific rate limit headers. |
...
| **[In Review]** | **Proof of Concept** | Build a working POC covering the critical path: Event Ingestion receives an OMS "order placed" event, Notification Engine processes it, Template Service renders an email, Channel Router Core delegates to the SendGrid adapter for delivery. Validates the architecture end-to-end. | Weeks 3-5 |
| **[In Review]** | **Core Service Implementation** | Develop the backend microservices (event-ingestion, notification-engine, template-service, channel-router core, provider adapters, admin-service, audit-service) with full functionality, Jest unit tests (>=80% coverage), and integration tests. See [05 -- Testing Strategy](05-testing-strategy.md). | Weeks 5-12 |
