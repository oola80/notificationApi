...
describe('CriticalEventConsumer', () => {
  let consumer: CriticalEventConsumer;
  ...
  const mockMessage: NormalizedEventMessage = {
    ...
    priority: 'critical',
    normalizedPayload: { orderId: '12345', amount: 99.99 },
  ...
  beforeEach(async () => {
    ...
    consumer = module.get<CriticalEventConsumer>(CriticalEventConsumer);
  ...
  it('should delegate to pipeline on success', async () => {
    await consumer.handleCriticalEvent(mockMessage, mockAmqpMsg);

    expect(pipeline.processEvent).toHaveBeenCalledWith(mockMessage);
  });

  it('should return undefined (ACK) on success', async () => {
    const result = await consumer.handleCriticalEvent(mockMessage, mockAmqpMsg);
  ...
  it('should republish on retryable error', async () => {
    pipeline.processEvent.mockRejectedValue(new Error('Connection timeout'));
