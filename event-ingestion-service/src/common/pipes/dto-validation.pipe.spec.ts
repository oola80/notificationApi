...
describe('DtoValidationPipe', () => {
  ...
  it('should throw HttpException with EIS-001 for missing required fields', async () => {
    ...
    try {
      ...
      expect(response.code).toBe('EIS-001');
      expect(response.details).toBe('INVALID_REQUEST_BODY');
    }
  });

  it('should reject non-whitelisted properties with EIS-001 error', async () => {
    try {
