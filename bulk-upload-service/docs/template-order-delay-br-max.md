# Bulk Upload Template: order.delay.br.max

> **Scenario:** Send delay notifications for MAX brand (Guatemala) orders that will not arrive on the original promise date. Variant of `order.delay` with 3 additional fields: `newPromiseDate`, `sellerId`, `sellerName`. Each order may contain multiple line items. The XLSX uses **group mode** to collapse multiple rows (one per item) into a single event per order. Notifications are sent via **email** and **whatsapp** channels.

## XLSX Column Layout

| # | Column | Type | Required | Description |
|---|--------|------|----------|-------------|
| 1 | `eventType` | string | Yes | Must be `order.delay.br.max` for every row |
...
| 13 | `promiseDate` | string | No | Original estimated delivery date in ISO 8601 format |
| 14 | `newPromiseDate` | string | No | **New** estimated delivery date after delay (ISO 8601) |
| 15 | `sellerId` | string | No | **New** Seller identifier |
| 16 | `sellerName` | string | No | **New** Seller display name |
| 17 | `item.sku` | string | Yes | Item SKU — `item.` prefix marks this as an item-level field |
| 18 | `item.name` | string | Yes | Item display name |
| 19 | `item.quantity` | number | Yes | Quantity ordered |
| 20 | `item.price` | number | Yes | Unit price |
...
> **New vs order.delay:** Columns 14-16 (`newPromiseDate`, `sellerId`, `sellerName`) are additions specific to this MAX brand variant. All other columns are identical to the `order.delay` template.
...
The test file `test-order-delay.br.max.xlsx` contains 3 data rows that produce 2 events:
...
order.delay.br.max   | 123132  | omar-ola@hotmail7.com   | omar-ola@hotmail.com    | Omar Ola     | 611214544-A   | 40009954 | Lamentamos (...) | [{...}] | FD15454        | FORZA_GT  | https://www.forzadelivery.com/gt/  | 2026-02-25T06:10:00Z     | 2026-02-28T06:10:00Z     | SEL-001  | Electrodomesticos GT | RUSH     | Ventilador de piso y pared Taurus de 20 pulgadas | 1             | 79.99
order.delay.br.max   | 123132  | omar-ola@hotmail7.com   | omar-ola@hotmail.com    | Omar Ola     | 611214544-A   | 40009954 | Lamentamos (...) | [{...}] | FD15454        | FORZA_GT  | https://www.forzadelivery.com/gt/  | 2026-02-25T06:10:00Z     | 2026-02-28T06:10:00Z     | SEL-001  | Electrodomesticos GT | FAN-20   | Ventilador de techo industrial 52 pulgadas        | 2             | 129.99
order.delay.br.max   | 123132  | maria.garcia@example.com| maria.garcia@example.com| Maria Garcia | 611214544-B   | 50012345 | Lamentamos (...) | [{...}] | FD15499        | FORZA_GT  | https://www.forzadelivery.com/gt/  | 2026-02-26T08:00:00Z     | 2026-03-01T08:00:00Z     | SEL-002  | Tech Store GT        | TV-55    | Smart TV LED 55 pulgadas 4K                       | 1             | 499.99
```
...
```json
{
  ...
  "sourceEventId": "{uploadId}-group-order.delay.br.max-611214544-A",
  ...
  "payload": {
    ...
    "newPromiseDate": "2026-02-28T06:10:00Z",
    "sellerId": "SEL-001",
    "sellerName": "Electrodomesticos GT",
    "items": [
      {
        "sku": "RUSH",
...
| **Dates** (`promiseDate`, `newPromiseDate`) | Enter as an ISO 8601 string (e.g. `2026-02-25T06:10:00Z`). If Excel auto-converts to a Date object, the parser converts it back to an ISO string. |
