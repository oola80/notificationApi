...
channel-router-service/
  ...
  src/
    ...
    delivery/
      ...
      delivery-pipeline.service.ts   # 7-step delivery orchestrator (validate, resolve, CB, rate limit, media, send, handle), stores sentPayload in metadata
      delivery-pipeline.service.spec.ts
      delivery-attempts.controller.ts  # GET /delivery-attempts/:notificationId — query delivery attempts by notification
      delivery-attempts.controller.spec.ts
