...
@Injectable()
export class EmailCriticalConsumer extends BaseDeliveryConsumer {
  protected readonly logger = new Logger(EmailCriticalConsumer.name);
  protected readonly channel = 'email';
  protected readonly priority = 'critical';
  protected readonly queueName = QUEUE_DELIVER_EMAIL_CRITICAL;
  ...
  @RabbitSubscribe({
    ...
    routingKey: 'notification.deliver.critical.email',
    queue: QUEUE_DELIVER_EMAIL_CRITICAL,
    queueOptions: {
      durable: true,
