...
@Injectable()
export class PushCriticalConsumer extends BaseDeliveryConsumer {
  protected readonly logger = new Logger(PushCriticalConsumer.name);
  protected readonly channel = 'push';
  protected readonly priority = 'critical';
  protected readonly queueName = QUEUE_DELIVER_PUSH_CRITICAL;
  ...
  @RabbitSubscribe({
    ...
    routingKey: 'notification.deliver.critical.push',
    queue: QUEUE_DELIVER_PUSH_CRITICAL,
    queueOptions: {
      durable: true,
