...
describe('Braze Adapter — Health, Capabilities & Metrics (e2e)', () => {
  ...
  describe('GET /health', () => {
    it('should return 200 with correct shape', async () => {
      ...
      expect(res.body).toHaveProperty('providerId', 'braze');
      expect(res.body).toHaveProperty('providerName', 'Braze');
      expect(res.body).toHaveProperty('supportedChannels');
      expect(res.body.supportedChannels).toContain('email');
      expect(res.body.supportedChannels).toContain('sms');
      expect(res.body.supportedChannels).toContain('whatsapp');
      expect(res.body.supportedChannels).toContain('push');
  ...
  describe('GET /capabilities', () => {
    it('should return 200 with Braze capabilities', async () => {
      const res = await request(app.getHttpServer())
        .get('/capabilities')
        .expect(200);
